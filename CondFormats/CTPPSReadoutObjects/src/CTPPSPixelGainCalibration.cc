...
float CTPPSPixelGainCalibration::getPed(const int& col, const int& row ) const {
  ...
  if (col >= nCols || row >= nRows){
    throw cms::Exception("CorruptedData")
      << "[CTPPSPixelGainCalibration::getPed] Pixel out of range: col " << col << " (" << nCols << ")" << " row " << row << " (" << nRows << ")";
  }
...
float CTPPSPixelGainCalibration::getGain(const int& col, const int& row ) const {
  ...
  if (col >= nCols || row >= nRows){
    throw cms::Exception("CorruptedData")
      << "[CTPPSPixelGainCalibration::getGain] Pixel out of range: col " << col << " (" << nCols << ")" <<" row " << row << " (" << nRows << ")" << " plane " << int((indexes.detid>>16) & 0X7);
  }  
