...
#include "CTPPSTools/Utilities/interface/CTPPSUnitConversion.h"
...
void ProtonTransport::addPartToHepMC( HepMC::GenEvent * evt )
{
    ...
    for (it = m_beamPart.begin(); it != m_beamPart.end(); ++it ) {
        ...
        double ddd=(direction>0)?fCTPPSRegionStart_45:fabs(fCTPPSRegionStart_56); // Totem uses negative Z for sector 56 while Hector uses always positive distance
