#include "CTPPSTools/Utilities/interface/CTPPSOpticsCalibrator.h"
#include <TEllipse.h>
#include <iomanip>
//using namespace std;
int main(int argc, char** argv)
{
    ...
    CTPPSOpticsCalibrator* ctpps_optc = new CTPPSOpticsCalibrator(Beam1FileName,Beam2FileName,BeamL);
    CTPPSTools::fBeamEnergy=BeamE;
    CTPPSTools::fBeamMomentum=sqrt(BeamE*BeamE-CTPPSTools::ProtonMassSQ);
    CTPPSTools::fCrossingAngleBeam1=Xangle1; // Boost is done in CMS ref frame, in which Xangle is negative
    CTPPSTools::fCrossingAngleBeam2=Xangle2; // ibdem
...
CTPPSOpticsCalibrator::CTPPSOpticsCalibrator(const std::string b1,const std::string b2, int len):beamline_length(len+0.1)
{
        IP_Found=false;
        ParIdx_Found=false;
        Emmitance_OK=false;
...
void CTPPSOpticsCalibrator::AlignBeamLine()
{
...
void CTPPSOpticsCalibrator::CalibrateBeamPositionatIP(double& xpos,double& ypos)
{
     xpos=fBeamXatIP;
     xpos=fBeamYatIP;
...
//
     ...
     while(1) {
        ...
        CTPPSTools::LorentzBoost(h_pp,1,"LAB");
        CTPPSTools::LorentzBoost(h_pn,-1,"LAB");
...
void CTPPSOpticsCalibrator::BeamProfile(TFile* fout,int Nevents)
{
...
// Define the index to the position where the profile is to be drawn
     ...
     for(int j=0;j<2;j++) {
        ...
        switch(j) {
              case 0:  // positive side
                     ...
                     beamline= &(*m_beamline45);   // CTPPS1 corresponds to beam2
                     DetPos = &(PosP);
                     direction=1;
                     break;
              case 1: // negative side
                     prof1 = &(*bp1b);prof2 = &(*bp2b); prof3 = &(*bp3b);
                     beamline= &(*m_beamline56);   // CTPPS2 corresponds to beam1
                     DetPos = &(PosN);
                     direction=-1;
                     break;
        }
        ...
        for(int i=0;i<Nevents;i++) {
           ...
           CTPPSTools::LorentzBoost(h_p,direction,"LAB");
     ...
     std::cout << "HectorForCTPPS: BEAM parameters (in um):\n" 
...
void CTPPSOpticsCalibrator::ReadEmittance(std::ifstream& tabfile)
{
     if (!tabfile.good()) {
...
void CTPPSOpticsCalibrator::ReadParameterIndex(std::ifstream& tabfile)
{
     if (!tabfile.good()) {
        std::cout << "Bad TWISS file. Could not read emittance."<<std::endl;
        return;
...
void CTPPSOpticsCalibrator::FindIP(std::ifstream& tabfile)
{
    if (!tabfile.good()) {
...
void CTPPSOpticsCalibrator::ReadBeamPositionFromOpticsFile(std::ifstream& tabfile)
{
...
void CTPPSOpticsCalibrator::AlignObject(H_OpticalElement* opt,H_BeamLine* bline,double init_displacement)
{
     double tolerance=1.;
     for(int i:{'x','y'}) {
         H_BeamParticle h_p;
