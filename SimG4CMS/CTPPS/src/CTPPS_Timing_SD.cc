...
CTPPS_Timing_SD::CTPPS_Timing_SD(const std::string & name, const DDCompactView & cpv, const SensitiveDetectorCatalog & clg,
...
void CTPPS_Timing_SD::Initialize(G4HCofThisEvent * HCE) {
    ...
    theHC = new CTPPS_Timing_G4HitCollection(GetName(), collectionName[0]);
...
uint32_t CTPPS_Timing_SD::setDetUnitId(const G4Step * aStep)
{ 
...
{
  LogDebug("PP_Timing_SD") << "CTPPS_Timing_SD: Collection " << theHC->GetName() << std::endl;
  theHC->PrintAllHits();
}

void CTPPS_Timing_SD::fillHits(edm::PSimHitContainer& c, const std::string & n) {
